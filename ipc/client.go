...
// Subscribe opens a streaming subscription for role ("bridge" | "errands") and
// calls handler for each incoming message event until the connection drops
// (then it returns the error). Callers typically run this in a reconnect loop.
func (c *Client) Subscribe(role string, handler func(Event)) error {
	...
	defer conn.Close()
	line, _ := json.Marshal(Request{Op: "subscribe", Role: role})
